...
namespace RimWorld
{
	public abstract class Designator_Place : Designator
	{
		...
		[CompilerGenerated]
		private sealed class <DoExtraGuiControls>c__AnonStorey0
		{
			internal Rect winRect;

			internal Designator_Place $this;

			public <DoExtraGuiControls>c__AnonStorey0()
			{
			}
			...
			internal void <>m__0()
			{
				...
				Text.Anchor = TextAnchor.MiddleCenter;
				Text.Font = GameFont.Medium;
				Rect rect = new Rect(this.winRect.width / 2f - 64f - 5f, 15f, 64f, 64f);
				if (Widgets.ButtonImage(rect, TexUI.RotLeftTex))
				{
					SoundDefOf.AmountDecrement.PlayOneShotOnCamera(null);
					rotationDirection = RotationDirection.Counterclockwise;
				...
				Widgets.Label(rect, KeyBindingDefOf.Designator_RotateLeft.MainKeyLabel);
				Rect rect2 = new Rect(this.winRect.width / 2f + 5f, 15f, 64f, 64f);
				if (Widgets.ButtonImage(rect2, TexUI.RotRightTex))
				{
					SoundDefOf.AmountIncrement.PlayOneShotOnCamera(null);
					rotationDirection = RotationDirection.Clockwise;
				...
				if (rotationDirection != RotationDirection.None)
				{
					this.$this.placingRot.Rotate(rotationDirection);
				}
				Text.Anchor = TextAnchor.UpperLeft;
				Text.Font = GameFont.Small;
			}
