...
namespace RimWorld
{
	public class WorkGiver_GrowerSow : WorkGiver_Grower
	{
		...
		public override Job JobOnCell(Pawn pawn, IntVec3 c)
		{
			...
			{
				if (!pawn.CanReserve(plant, 1, -1, ReservationLayer.Default, false) || plant.IsForbidden(pawn))
				{
					return null;
				}
			...
			else
			{
				...
				if (thing2 != null)
				{
					...
					if (plant2 != null && pawn.CanReserve(plant2, 1, -1, ReservationLayer.Default, false) && !plant2.IsForbidden(pawn))
					{
				...
				while (j < thingList.Count)
				{
					...
					{
						if (!pawn.CanReserve(thing3, 1, -1, ReservationLayer.Default, false))
						{
							return null;
						}
				...
				if (!WorkGiver_Grower.wantedPlantDef.CanEverPlantAt(c, pawn.Map) || !GenPlant.GrowthSeasonNow(c, pawn.Map) || !pawn.CanReserve(c, 1, -1, ReservationLayer.Default, false))
				{
					return null;
				}
