...
namespace RimWorld
{
	public static class HealthAIUtility
	{
		...
		public static Thing FindBestMedicine(Pawn healer, Pawn patient)
		{
			...
			Predicate<Thing> predicate = (Thing m) => !m.IsForbidden(healer) && patient.playerSettings.medCare.AllowsMedicine(m.def) && healer.CanReserve(m, 1, -1, ReservationLayer.Default, false);
