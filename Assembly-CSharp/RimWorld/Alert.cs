...
namespace RimWorld
{
	...
	public abstract class Alert
	{
		...
		[CompilerGenerated]
		private sealed class <DrawInfoPane>c__AnonStorey0
		{
			internal Rect infoRect;

			internal string expString;

			public <DrawInfoPane>c__AnonStorey0()
			{
			}
			...
			{
				Text.Font = GameFont.Small;
				Rect rect = this.infoRect.AtZero();
				Widgets.DrawWindowBackground(rect);
				Rect position = rect.ContractedBy(10f);
				GUI.BeginGroup(position);
				Widgets.Label(new Rect(0f, 0f, position.width, position.height), this.expString);
				GUI.EndGroup();
			}
		}
	}
