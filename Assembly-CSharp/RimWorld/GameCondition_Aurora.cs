...
namespace RimWorld
{
	public class GameCondition_Aurora : GameCondition
	{
		...
		private float Brightness
		{
			get
			{
				return (float)((!base.Permanent) ? 0.73000001907348633 : Mathf.Lerp(0.73f, 1f, GenCelestial.CurCelestialSunGlow(base.Map)));
			}
		}
		...
		{
			this.curColorTransition += (float)(1.0 / (float)this.TransitionDurationTicks);
			if (this.curColorTransition >= 1.0)
			{
				this.prevColorIndex = this.curColorIndex;
				this.curColorIndex = this.GetNewColorIndex();
				this.curColorTransition = 0f;
			}
			if (!base.Permanent && base.TicksLeft > 200 && GenCelestial.CurCelestialSunGlow(base.Map) > 0.5)
			{
				base.TicksLeft = 200;
			}
		}
