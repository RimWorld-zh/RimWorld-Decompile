...
namespace RimWorld
{
	public static class FloatMenuMakerMap
	{
		...
		private static void AddDraftedOrders(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
		{
			...
			{
				FloatMenuMakerMap.<AddDraftedOrders>c__AnonStorey42A <AddDraftedOrders>c__AnonStorey42A = new FloatMenuMakerMap.<AddDraftedOrders>c__AnonStorey42A();
				<AddDraftedOrders>c__AnonStorey42A.attackTarg = current;
				if (pawn.equipment.Primary != null && !pawn.equipment.PrimaryEq.PrimaryVerb.verbProps.MeleeRange)
				{
					string str;
					Action rangedAct = FloatMenuUtility.GetRangedAttackAction(pawn, <AddDraftedOrders>c__AnonStorey42A.attackTarg, out str);
					string text = "FireAt".Translate(new object[]
					{
						<AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing.Label
					});
					...
					else
					{
						...
						{
							MoteMaker.MakeStaticMote(<AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing.DrawPos, <AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing.Map, ThingDefOf.Mote_FeedbackAttack, 1f);
							rangedAct();
						};
					}
				...
				string str2;
				Action meleeAct = FloatMenuUtility.GetMeleeAttackAction(pawn, <AddDraftedOrders>c__AnonStorey42A.attackTarg, out str2);
				Pawn pawn2 = <AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing as Pawn;
				string text2;
				...
				MenuOptionPriority priority = (!<AddDraftedOrders>c__AnonStorey42A.attackTarg.HasThing || !pawn.HostileTo(<AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing)) ? MenuOptionPriority.VeryLow : MenuOptionPriority.AttackEnemy;
				Thing thing = <AddDraftedOrders>c__AnonStorey42A.attackTarg.Thing;
				...
				foreach (LocalTargetInfo current2 in GenUI.TargetsAt(clickPos, TargetingParameters.ForArrest(pawn), true))
				{
					...
					if (!((Pawn)dest.Thing).Downed)
					{
						...
						else
						{
							...
							{
								Building_Bed building_Bed = RestUtility.FindBedFor(pTarg, pawn, true, false);
				...
				foreach (LocalTargetInfo current2 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), true))
				{
					...
					if (!victim.InBed() && pawn.CanReserveAndReach(victim, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, false))
					{
						if ((victim.Faction == Faction.OfPlayer && victim.MentalStateDef == null) || (victim.Faction != Faction.OfPlayer && victim.MentalStateDef == null && !victim.IsPrisonerOfColony && (victim.Faction == null || !victim.Faction.HostileTo(Faction.OfPlayer))))
						{
							...
							{
								Building_Bed building_Bed = RestUtility.FindBedFor(victim, pawn, false, false);
								if (building_Bed == null)
								{
									string str2;
