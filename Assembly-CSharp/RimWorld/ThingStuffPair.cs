...
namespace RimWorld
{
	public struct ThingStuffPair : IEquatable<ThingStuffPair>
	{
		...
		[CompilerGenerated]
		private static float <AllWith>m__0(ThingDef st)
		{
			return st.stuffProps.commonality;
		}

		[CompilerGenerated]
		private static float <AllWith>m__1(ThingStuffPair p)
		{
			return p.Price;
		}

		[CompilerGenerated]
		private sealed class <AllWith>c__AnonStorey0
		{
			internal ThingDef thingDef;

			public <AllWith>c__AnonStorey0()
			{
			}

			internal bool <>m__0(ThingDef st)
			{
				return st.IsStuff && st.stuffProps.CanMake(this.thingDef);
			}
		}
	}
}
