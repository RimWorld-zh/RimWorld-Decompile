...
namespace RimWorld
{
	...
	internal class Gizmo_EnergyShieldStatus : Gizmo
	{
		...
		private sealed class <GizmoOnGUI>c__AnonStorey0
		{
			...
			internal Gizmo_EnergyShieldStatus $this;
			...
			internal void <>m__0()
			{
				...
				Text.Font = GameFont.Tiny;
				Widgets.Label(rect2, this.$this.shield.LabelCap);
				Rect rect3 = rect;
				rect3.yMin = this.overRect.height / 2f;
				float fillPercent = this.$this.shield.Energy / Mathf.Max(1f, this.$this.shield.GetStatValue(StatDefOf.EnergyShieldEnergyMax, true));
				Widgets.FillableBar(rect3, fillPercent, Gizmo_EnergyShieldStatus.FullShieldBarTex, Gizmo_EnergyShieldStatus.EmptyShieldBarTex, false);
				Text.Font = GameFont.Small;
				Text.Anchor = TextAnchor.MiddleCenter;
				Widgets.Label(rect3, (this.$this.shield.Energy * 100f).ToString("F0") + " / " + (this.$this.shield.GetStatValue(StatDefOf.EnergyShieldEnergyMax, true) * 100f).ToString("F0"));
				Text.Anchor = TextAnchor.UpperLeft;
			}
		}
	}
