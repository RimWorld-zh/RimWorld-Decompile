...
namespace RimWorld
{
	public class WorkGiver_BuildRoof : WorkGiver_Scanner
	{
		...
		{
			return pawn.Map.areaManager.BuildRoof[c] && !c.Roofed(pawn.Map) && pawn.CanReserve(c, 1, -1, ReservationLayer.Ceiling, false) && (pawn.CanReach(c, PathEndMode.Touch, pawn.NormalMaxDanger(), false, TraverseMode.ByPawn) || this.BuildingToTouchToBeAbleToBuildRoof(c, pawn) != null) && RoofCollapseUtility.WithinRangeOfRoofHolder(c, pawn.Map) && RoofCollapseUtility.ConnectedToRoofHolder(c, pawn.Map, true);
		}
