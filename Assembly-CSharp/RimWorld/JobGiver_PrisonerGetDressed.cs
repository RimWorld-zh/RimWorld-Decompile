			...
			if (room.isPrisonCell)
			{
				foreach (IntVec3 current in room.Cells)
				{
					...
					for (int i = 0; i < thingList.Count; i++)
					{
						...
						if (apparel != null && apparel.def.apparel.bodyPartGroups.Contains(bodyPartGroupDef) && pawn.CanReserve(apparel, 1, -1, ReservationLayer.Default, false) && ApparelUtility.HasPartsToWear(pawn, apparel.def))
						{
							return apparel;
						}
					}
