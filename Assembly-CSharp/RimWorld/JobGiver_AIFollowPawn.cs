...
namespace RimWorld
{
	public abstract class JobGiver_AIFollowPawn : ThinkNode_JobGiver
	{
		...
		protected override Job TryGiveJob(Pawn pawn)
		{
			...
			if ((followee.pather.Moving && (float)followee.pather.Destination.Cell.DistanceToSquared(pawn.Position) > radius * radius) || followee.GetRoom(RegionType.Set_Passable) != pawn.GetRoom(RegionType.Set_Passable) || (float)followee.Position.DistanceToSquared(pawn.Position) > radius * radius)
			{
				IntVec3 root;
				...
				IntVec3 c = CellFinder.RandomClosewalkCellNear(root, followee.Map, Mathf.RoundToInt(radius * 0.7f), null);
