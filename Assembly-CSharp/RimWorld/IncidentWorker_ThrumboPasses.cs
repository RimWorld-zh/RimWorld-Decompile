...
namespace RimWorld
{
	public class IncidentWorker_ThrumboPasses : IncidentWorker
	{
		...
		{
			Map map = (Map)target;
			return (byte)((!map.gameConditionManager.ConditionIsActive(GameConditionDefOf.ToxicFallout)) ? 1 : 0) != 0;
		}
		...
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
			...
			else
			{
				...
				Find.LetterStack.ReceiveLetter("LetterLabelThrumboPasses".Translate(thrumbo.label).CapitalizeFirst(), "LetterThrumboPasses".Translate(thrumbo.label), LetterDefOf.PositiveEvent, (Thing)pawn, (string)null);
				result = true;
			}
			return result;
		}
