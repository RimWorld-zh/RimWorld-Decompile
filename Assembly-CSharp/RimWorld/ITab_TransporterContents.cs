...
namespace RimWorld
{
	public class ITab_TransporterContents : ITab
	{
		...
		private void DoThingRow(ThingDef thingDef, int count, List<Thing> things, float width, ref float curY, Action<int> discardAction)
		{
			...
			if (count != 1)
			{
				...
				if (Widgets.ButtonImage(butRect, CaravanThingsTabUtility.AbandonSpecificCountButtonTex))
				{
					...
					{
						thingDef.label
					}), 1, count, discardAction, int.MinValue));
				}
			}
			rect.width -= 24f;
		...
		static ITab_TransporterContents()
		{
		}

		[CompilerGenerated]
		private sealed class <DoItemsLists>c__AnonStorey0
		{
			internal TransferableOneWay t;

			internal ITab_TransporterContents $this;

			public <DoItemsLists>c__AnonStorey0()
			{
			}

			internal void <>m__0(int x)
			{
				this.t.ForceTo(this.t.CountToTransfer - x);
				this.$this.EndJobForEveryoneHauling(this.t);
			}
		}

		[CompilerGenerated]
		private sealed class <DoItemsLists>c__AnonStorey1
		{
			internal Thing t;

			internal ITab_TransporterContents $this;

			public <DoItemsLists>c__AnonStorey1()
			{
			}
			...
			{
				Thing thing;
				GenDrop.TryDropSpawn(this.t.SplitOff(x), this.$this.SelThing.Position, this.$this.SelThing.Map, ThingPlaceMode.Near, out thing, null, null);
			}
		}

		[CompilerGenerated]
		private sealed class <DoThingRow>c__AnonStorey2
		{
			internal Action<int> discardAction;

			internal int count;

			public <DoThingRow>c__AnonStorey2()
			{
			}

			internal void <>m__0()
			{
				this.discardAction(this.count);
			}
		}
	}
}
