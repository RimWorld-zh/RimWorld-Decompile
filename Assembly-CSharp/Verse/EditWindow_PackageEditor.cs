	...
	public class EditWindow_PackageEditor<TNewDef> : EditWindow where TNewDef : Def, new()
	{
		...
		public override void DoWindowContents(Rect selectorInner)
		{
			...
			else
			{
				...
				{
					EditWindow_PackageEditor<TNewDef>.<DoWindowContents>c__AnonStorey5CA <DoWindowContents>c__AnonStorey5CA = new EditWindow_PackageEditor<TNewDef>.<DoWindowContents>c__AnonStorey5CA();
					<DoWindowContents>c__AnonStorey5CA.<>f__this = this;
					<DoWindowContents>c__AnonStorey5CA.deletingDef = null;
					foreach (Def def in this.curPackage)
					{
						if (listing_Standard.SelectableDef(def.defName, false, delegate
						{
							<DoWindowContents>c__AnonStorey5CA.deletingDef = def;
						}))
						{
							bool flag = false;
					...
					if (<DoWindowContents>c__AnonStorey5CA.deletingDef != null)
					{
						Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation("Really delete Def " + <DoWindowContents>c__AnonStorey5CA.deletingDef.defName + "?", delegate
						{
							<DoWindowContents>c__AnonStorey5CA.<>f__this.curPackage.RemoveDef(<DoWindowContents>c__AnonStorey5CA.deletingDef);
						}, true, null));
					}
				}
