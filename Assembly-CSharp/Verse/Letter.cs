...
namespace Verse
{
	public abstract class Letter : IArchivable, ILoadReferenceable, IExposable
	{
		...
		[CompilerGenerated]
		private sealed class <CheckForMouseOverTextAt>c__AnonStorey1
		{
			internal string mouseoverText;

			public <CheckForMouseOverTextAt>c__AnonStorey1()
			{
			}
		}

		[CompilerGenerated]
		private sealed class <CheckForMouseOverTextAt>c__AnonStorey0
		{
			internal Rect infoRect;

			internal Letter.<CheckForMouseOverTextAt>c__AnonStorey1 <>f__ref$1;

			public <CheckForMouseOverTextAt>c__AnonStorey0()
			{
			}
			...
			{
				Text.Font = GameFont.Small;
				Rect position = this.infoRect.AtZero().ContractedBy(10f);
				GUI.BeginGroup(position);
				Widgets.Label(new Rect(0f, 0f, position.width, position.height), this.<>f__ref$1.mouseoverText);
				GUI.EndGroup();
			}
		}
	}
