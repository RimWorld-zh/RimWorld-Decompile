...
namespace Verse
{
	public static class MapSerializeUtility
	{
		...
		{
			return DataSerializeUtility.SerializeUshort(map.info.NumCells, (Func<int, ushort>)((int idx) => shortReader(map.cellIndices.IndexToCell(idx))));
		}

		public static void LoadUshort(byte[] arr, Map map, Action<IntVec3, ushort> shortWriter)
		{
			DataSerializeUtility.LoadUshort(arr, map.info.NumCells, (Action<int, ushort>)delegate(int idx, ushort data)
			{
